...
function getRectangleString(w, h) {
  // throw new Error('Not implemented');
  const str = `┌${'─'.repeat(w - 2)}┐\n`;
  const str1 = `│${' '.repeat(w - 2)}│\n`.repeat(h - 2);
  const str2 = `└${'─'.repeat(w - 2)}┘\n`;
...
function encodeToRot13(str) {
  ...
  return str.split('').map((a) => rot13[arr.indexOf(a)]).join('');
}
...
function isString(val) {
  ...
  if (!val) return false;
  const a = val;
  if (typeof a === 'object') return (typeof a.valueOf() === 'string');
